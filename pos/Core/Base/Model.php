...
class Model
{
    ...
    {
        //this configration to connecte with database
    ...
    public function topfive($col, $table, $n)
    {
        $data = array();
     ...
     * @return array
     */
    public function lowQuantityItems()
    {
        $data = array();
        $result = $this->connection->query("SELECT * FROM items WHERE quantity <= 3 ORDER BY quantity DESC");
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_object()) {
                $data[] = $row;
            }
